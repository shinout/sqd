(function() {
  ...
  main = function(options) {
    var command, finishProcesses, input, nProcess, onClose, output, run, separationOpearator, separationRule, separator, separatorAsJS, sepcommand, startTime, tmpfiles, worker, workerProcess;
    input = options.input, output = options.output, command = options.command, separator = options.separator, nProcess = options.nProcess, workerProcess = options.workerProcess, startTime = options.startTime, onClose = options.onClose;
    if (!separator) {
      separator = "line";
    }
  ...
  exports.run = function() {
    ...
    try {
      ap = require("argparser").files(0).arglen(1, 2).vals("c", "command", "s", "sep").nonvals("w").defaults({
        p: 4
      }).parse();
    } catch (_error) {
      e = _error;
    ...
    return main({
      ...
      startTime: startTime,
      onClose: function() {
        return console.error("time: %dms", new Date().getTime() - startTime);
      }
    });
  };
