(function() {
  ...
  main = function(options) {
    var command, count, debug, finishProcesses, input, interval, mem, nProcess, onClose, output, rhwm, run, separationOpearator, separationRule, separator, separatorAsJS, sepcommand, startTime, stop, tmpfiles, whwm;
    input = options.input, output = options.output, command = options.command, separator = options.separator, nProcess = options.nProcess, startTime = options.startTime, onClose = options.onClose, debug = options.debug, stop = options.stop, mem = options.mem, rhwm = options.rhwm, whwm = options.whwm;
    if (mem) {
      count = 0;
  ...
  exports.run = function() {
    ...
    try {
      ap = require("argparser").files(0).arglen(1, 2).vals("c", "command", "s", "sep").nonvals("w", "debug", "d", "e", "exit", "mem").nums("rhwm", "whwm").defaults({
        p: 4
      }).parse();
    } catch (_error) {
      e = _error;
