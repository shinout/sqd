(function() {
  ...
  main = function(options) {
    var command, count, debug, finishProcesses, input, interval, mem, nProcess, onClose, output, run, separationOpearator, separationRule, separator, separatorAsJS, sepcommand, startTime, stop, tmpfiles;
    input = options.input, output = options.output, command = options.command, separator = options.separator, nProcess = options.nProcess, startTime = options.startTime, onClose = options.onClose, debug = options.debug, stop = options.stop, mem = options.mem;
    if (mem) {
      count = 0;
